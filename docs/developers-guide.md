...
## 9. Mutation-testing workflow contract tests
...
— running `cargo-mutants`, sharding, and summarising survivors — lives in
...
control) mutates the whole crate, fanned out across shards; select a branch in
that control to exercise a feature branch.

The caller passes a single configuration input, carrying intent:

- `extra-args` — arguments forwarded to `cargo-mutants` (here `--all-features`)
  so the mutation run matches the CI test baseline (`make test` runs
  `--workspace --all-targets --all-features`); a mismatch would report
  feature-gated code as untested.

The caller relies on the shared workflow's defaults for `paths` and
`exclude-globs`: mapsplice is a single root crate with all mutable source
under `src/` and no scaffolding modules outside `#[cfg(test)]`, so the default
change-detection globs and exclusions already fit.
...
Because the caller is configuration rather than code, the contract test in
...
feature-flag configuration — rather than letting the breakage surface only in
a scheduled run. Run it locally with `make test-workflow-contracts`. The test
validates:
